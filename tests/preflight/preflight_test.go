...
var _ = Describe("Preflight Tests", func() {
	...
	FContext("When checks are successful and volume snapshot class value is not provided as input", func() {
		var (
			once          sync.Once
	...
	FContext("When preflight checks are successful and volume snapshot class value is provided as input", func() {
		var (
			once          sync.Once
			outputLogData string
		)
	...
	Context("When preflight checks are executed successfully", func() {
		...
		It("Should clean volume snapshot source pod", func() {
			Expect(outputLogData).To(MatchRegexp("(Deleted source pod - source-pod-)([a-z]{6})"))
		})

		It("Should clean volume snapshot restore pod", func() {
			Expect(outputLogData).To(MatchRegexp("(Cleaning Pod - restored-pod-)([a-z]{6})"))
		})
	...
	Context("When image pull secret value is provided but local-registry value is not provided", func() {
		var outputLogData string
		var once sync.Once
		var progErr error
		...
		BeforeEach(func() {
			once.Do(func() {
				var byteData []byte
				inputFlags := flagsMap
				inputFlags[imagePullSecFlag] = "invalidImagePullSecret"
				err = cleanDirForFiles(preflightLogFilePrefix)
				Expect(err).To(BeNil())
				cmd := fmt.Sprintf("./tvk-preflight run --%s %s --%s %s",
					storageClassFlag, defaultTestStorageClass, imagePullSecFlag, "invalidImagePullSecret")
				_, progErr = shell.RunCmd(cmd)
				byteData, err = getLogFileData(preflightLogFilePrefix)
				Expect(err).To(BeNil())
				outputLogData = string(byteData)
			})
		...
		It("Program should not run and should exit", func() {
			Expect(outputLogData).
				To(ContainSubstring("Cannot give image pull secret if local registry is not provided"))
			Expect(progErr).To(Not(BeNil()))
		})
	})
...
func runPreflightChecks(flagsMap map[string]string) {
	...
	cmd := fmt.Sprintf("./tvk-preflight run %s", flags)
...
func runCleanupWithUID(uid string) {
	...
	Expect(err).To(BeNil())
	cmd := fmt.Sprintf("./tvk-preflight cleanup --uid %s", uid)
