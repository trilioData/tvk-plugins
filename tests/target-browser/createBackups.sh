  ...
  for ((j = 0; j < $2; j++)); do
    ...
    if [ "$3" = "backup" ]; then
      ...
      if [ "$6" == "resource-metadata" ]; then
        mkdir "${backuppath}"/operator
        mkdir "${backuppath}"/operator/metadata-snapshot/
        mv "${src_dir}"/test-data/resource-metadata.json "${backuppath}"/operator/metadata-snapshot/resource-metadata.json
      fi
