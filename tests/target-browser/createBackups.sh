...
for (( i=0; i<$1; i++ )); do
    ...
    for (( j=0; j<$2; j++ )); do
      ...
      elif [ "$3" = "create-failed-backup" ]
      then
        cp "${src_dir}"/test_files/backup-metasnapshot-failed.json "${src_dir}"/test_files/backup-modified.json
        cp "${src_dir}"/test_files/backupplan-metasnapshot-failed.json "${src_dir}"/test_files/backupplan-modified.json
        sed -i "s/NAME/failed-backup-$j/g" "${src_dir}"/test_files/backup-modified.json
        sed -i "s/BACKUP-UUID/$backupuid/g" "${src_dir}"/test_files/backup-modified.json
        sed -i "s/BACKUPPLAN-UUID/$bplanuid/g" "${src_dir}"/test_files/backup-modified.json
        sed -i "s/BACKUPPLAN-UUID/$bplanuid/g" "${src_dir}"/test_files/backupplan-modified.json
        sed -i "s/BACKUP-UUID/$backupuid/g" "${src_dir}"/test_files/backupplan-modified.json
