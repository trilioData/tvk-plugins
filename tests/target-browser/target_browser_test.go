	...
	Context("Resource Metadata command test-cases", func() {
		...
		Context("Filter Operations on absolute values for flags used", func() {
			...
			BeforeEach(func() {
				once.Do(func() {
					...
					output, _ := exec.Command(createBackupScript, strconv.Itoa(noOfBackupPlansToCreate),
						strconv.Itoa(noOfBackupsToCreatePerBackupPlan), "backup", backupUID, "custom", "resource-metadata").Output()
			...
			It("Should filter resource metadata on specific values of flags", func() {
				isLast = true
				args := []string{cmdGet, cmdResourceMetadata, flagGroup, "acid.zalan.do", flagKind, "postgresql",
					flagVersion, "v1", flagName, "acid-minimal-cluster", flagBackupUIDFlag, backupUID, flagBackupPlanUIDFlag, backupPlanUIDs[0]}
				args = append(args, commonArgs...)
				command := exec.Command(targetBrowserBinaryFilePath, args...)
				log.Info(command)
				output, err := command.CombinedOutput()
				...
				jsonFile, err := os.Open(filepath.Join(testDataDirRelPath, "resource-metadata.json"))
				Expect(err).To(BeNil())
	...
	Context("Trilio Resources subcommand test-cases", func() {
		...
		Context("Filter Operations on absolute values for flags used", func() {
			...
			It("Should filter trilio resources when backupPlan not given and only 1 kind given", func() {
				...
				re := regexp.MustCompile("(?m)[\r\n]+^.*location.*$")
				trilioResources := re.ReplaceAllString(string(output), "")
				jsonFile, err := os.Open(filepath.Join(testDataDirRelPath, "trilio-resources-backup.json"))
				Expect(err).To(BeNil())
				defer jsonFile.Close()
				expectedResourceMetadata, _ := ioutil.ReadAll(jsonFile)
				Expect(reflect.DeepEqual(expectedResourceMetadata, trilioResources))
			})
			...
			It("Should filter trilio resources when no kinds given", func() {
				...
				re := regexp.MustCompile("(?m)[\r\n]+^.*location.*$")
				trilioResources := re.ReplaceAllString(string(output), "")
				jsonFile, err := os.Open(filepath.Join(testDataDirRelPath, "trilio-resources-backup-backupplan.json"))
				Expect(err).To(BeNil())
				defer jsonFile.Close()
				expectedResourceMetadata, _ := ioutil.ReadAll(jsonFile)
				Expect(reflect.DeepEqual(expectedResourceMetadata, trilioResources))
