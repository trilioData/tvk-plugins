...
var _ = Describe("Preflight Helper Funcs Unit Tests", func() {
	...
	Context("Check cluster has snapshot class", func() {
		It("Should return volume-snapshot-class object when snapshotclass name is provided and is present on cluster", func() {
			_, err := clusterHasVolumeSnapshotClass(ctx, internal.DefaultTestSnapshotClass, contClient)
			Expect(err).To(BeNil())
		})

		It("Should return error when snapshot class does not exist on cluster", func() {
			_, err := clusterHasVolumeSnapshotClass(ctx, internal.InvalidSnapshotClassName, contClient)
			Expect(err).ToNot(BeNil())
			Expect(err.Error()).To(ContainSubstring(
				fmt.Sprintf("volume snapshot class %s not found on cluster ::",
					internal.InvalidSnapshotClassName)))
		})

		It("Should return error when runtime client object is nil", func() {
			_, err := clusterHasVolumeSnapshotClass(ctx, internal.DefaultTestSnapshotClass, nil)
			Expect(err).ToNot(BeNil())
			Expect(err.Error()).To(ContainSubstring(
	...
	Context("Waiting until volume snapshot becomes ready to use", func() {
		var (
			once           sync.Once
			snapshotVer    string
			resourceSuffix string
		...
		It("Should wait until volume snapshot becomes ready with given retry backoff parameters", func() {
			resourceSuffix, err = CreateResourceNameSuffix()
			Expect(err).To(BeNil())
			cops := cleanupOps
			...
			volSnap := createVolumeSnapsotSpec(testSnapPrefix+resourceSuffix, internal.DefaultTestSnapshotClass,
				runOps.Namespace, snapshotVer, pvc.GetName(), resourceSuffix)
		...
		It("Should return error when pvc does not exist", func() {
			resourceSuffix, err = CreateResourceNameSuffix()
			Expect(err).To(BeNil())
			volSnap := createVolumeSnapsotSpec(testSnapPrefix+resourceSuffix, internal.DefaultTestSnapshotClass,
				runOps.Namespace, snapshotVer, testPVCPrefix+resourceSuffix, resourceSuffix)
			err = runtimeClient.Create(ctx, volSnap)
			Expect(err).To(BeNil())
	...
	Context("Execute shell commands in the containers of a pod", func() {
		var (
			resourceSuffix string
			err            error
			pod            *corev1.Pod
		...
		BeforeEach(func() {
			execOp = &exec.Options{
				...
				Config:    restConfigClient,
				ClientSet: k8sClient,
			}
		})
		It("Should execute shell command in the busybox container of a running pod", func() {
			resourceSuffix, err = CreateResourceNameSuffix()
			Expect(err).To(BeNil())
			_, pod, err = runOps.createSourcePodAndPVC(ctx, resourceSuffix)
			Expect(err).To(BeNil())
			cops := cleanupOps
			cops.UID = resourceSuffix
			defer cops.CleanupPreflightResources(ctx)

			execOp.PodName = pod.GetName()
			execOp.ContainerName = pod.Spec.Containers[0].Name
			err = execInPod(execOp, runOps.Logger)
			Expect(err).To(BeNil())
		})

		It("Should return err if executed command in container exits with error", func() {
			resourceSuffix, err = CreateResourceNameSuffix()
			Expect(err).To(BeNil())
			_, pod, err = runOps.createSourcePodAndPVC(ctx, resourceSuffix)
			Expect(err).To(BeNil())
			...
			execOp.PodName = pod.GetName()
			execOp.ContainerName = pod.Spec.Containers[0].Name
			execOp.Command = []string{"/bin/sh", "-c", "cat /demo/non-existent-file.txt"}
			err = execInPod(execOp, runOps.Logger)
			Expect(err).ToNot(BeNil())
		})

		It("Should return err when pod does not exist in the given namespace", func() {
			execOp.PodName = testPodPrefix + resourceSuffix
			err = execInPod(execOp, runOps.Logger)
			Expect(err).ToNot(BeNil())
		})
	})
