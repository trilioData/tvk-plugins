...
func NewRuntimeSorter(objects []runtime.Object, sortBy string) *RuntimeSorter {
	parsedField, err := get.RelaxedJSONPathExpression(sortBy)
	if err != nil {
		parsedField = sortBy
	}
...
//nolint // Regex has their own use, If changed might have there functionality got hampered
...
func (s *SortingPrinter) sortObj(obj runtime.Object) error {
	...
	//nolint // obj.(type) can only be used with switch
...
//nolint // Function has their own use, If changed might have there functionality got hampered
...
func (r *RuntimeSort) Less(i, j int) bool {
	...
	iField := iValues[0][0]
	jField := jValues[0][0]
	// Code Added for handling nil cases
	if iField.IsZero() && jField.IsZero() {
		return false
	} else if iField.IsZero() {
		return true
	} else if jField.IsZero() {
		return false
	}
