...
func HandleInternalError(response *restful.Response, err error, parseError bool) {
	...
	var value interface{}
	if parseError {
		value = common.GetErrorMap(err)
	} else {
		value = map[string][]string{"message": {err.Error()}}
	}
	_ = response.WriteHeaderAndEntity(statusCode, value)
}
