...
helm_install() {
  ...
  if [[ -n "${UPDATE_INGRESS}" ]]; then
    ...
    instance_info=$(gcloud compute instances describe "$node" --zone "$(kubectl get nodes "$node" --show-labels --no-headers | awk -F"topology.kubernetes.io/zone=" '{print $2}')" --format=json | jq '.| "\(.tags.items[0]) \(.networkInterfaces[].network)"')
